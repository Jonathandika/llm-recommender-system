...
class RecommendationSystemRL:
    ...
    states = '/data/TFIDF-states.csv'
    books = '/data/booksamples.csv'
    ...
    def __init__(self, retrain=False):
        ...
        if not retrain:
            ...
            #CHANGE TO YOUR PATH
            self.rec_initial_full = pd.read_csv('/output/rec_initial.csv')
            self.train_data = pd.read_csv('/data/traindata_appended.csv')
            ...
            #CHANGE TO YOUR PATH
            self.train_data = pd.read_csv('/data/traindata.csv')
    ...
    def _copy_model(self, model):
        ...
        """
        model.save('/output/tmp_model')
        target_model = keras.models.load_model('/output/tmp_model')
        return target_model
    ...
    def get_initial_recommendation(self):
        ...
        for i in range(len(user_ids)):
            ...
            for j in range(len(self.train_data['user_encoded'])):
                if  user_ids[i] == self.train_data['user_encoded'][j]:
                    ...
                    user = pd.read_csv('/data/user_' + x + '.csv')
                    done = False
                    flag = 0
        ...
        self.train_data.to_csv('/data/traindata_appended.csv')
        return rec_initial
...
if __name__ == "__main__":
    ...
    rec_new.to_csv('output/rec_new.csv', index=False)